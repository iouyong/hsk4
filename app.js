...
const SCREENS = ['select-screen','app-screen','exam-screen','quiz-screen','result-screen','fc-setup-screen','fc-play-screen','fc-end-screen','pt-setup-screen','pt-play-screen','pt-result-screen','py-setup-screen','py-play-screen'];
...
function restoreAppState(){
  try{
    ...
    if(['py-setup-screen','py-play-screen'].includes(state.screen)){
      showPySetup();
      return true;
    }
...
function startPt(){
  ...
  var pool = [];
  for(var i=0; i<VOCAB.length; i++){
    if(ptCfg.pool==='starred' && !starred.has(i)) continue;
    if(ptCfg.pool==='unstarred' && starred.has(i)) continue;
    pool.push(i);
  }
  if(!pool.length){
  ...
  ptCards = pool.slice(0, Math.min(ptCfg.count || pool.length, pool.length));
  ptIdx = 0;
  ptResults = [];
...
var pyCfg = { count: 10, order: 'seq' };
var pyCards = [], pyIdx = 0, pyResults = [];

function showPySetup(){
  // initialize options
  pyCfg = { count: 10, order: 'seq' };
...
function startPy(){
  var pool = [];
  for(var i=0;i<VOCAB.length;i++) pool.push(i);
  if(pool.length===0){ alert('Không có từ để kiểm tra!'); return; }
  if(pyCfg.order === 'rand'){
    for(var i=pool.length-1;i>0;i--){ var j=Math.floor(Math.random()*(i+1)); var t=pool[i]; pool[i]=pool[j]; pool[j]=t; }
  }
  var cnt = Math.min(pyCfg.count || 10, pool.length);
...
function pyRender(){
  if(pyIdx >= pyCards.length){
    // finish: show result simple alert or go back
    var correct = pyResults.filter(r=>r.correct).length;
    alert('Kết thúc: đúng ' + correct + '/' + pyResults.length + ' từ');
    backToApp();
    return;
  }
